...
// #[serde(tag = "type")]
// pub enum CommentDef<S> where S: AsRef<str> {
//     Comment {
...
// fn serialize_comment_content<'se, R, S>(v: &Vec<S>, serializer: R) -> Result<R::Ok, R::Error>
// where
//     R: Serializer,
//     S: AsRef<str>
// {
//     let v: Vec<&str> = v.iter().map(|v| v.as_ref()).collect();
...
// fn serialize_identifier_option<'se, R>(
//     v: &Option<ast::Identifier<String>>,
//     serializer: R,
// ) -> Result<R::Ok, R::Error>
// where
//     R: Serializer,
// {
//     #[derive(Serialize)]
//     struct Helper<'ast>(#[serde(with = "IdentifierDef")] &'ast ast::Identifier<String>);
//     v.as_ref().map(Helper).serialize(serializer)
// }
