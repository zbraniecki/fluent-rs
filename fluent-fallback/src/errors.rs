...
pub enum LocalizationError {
    Bundle { id: Option<String>, error: FluentError },
    MissingMessage { id: String },
    MissingValue { id: String },
    SyncRequestInAsyncMode { id: String },
}
...
impl std::fmt::Display for LocalizationError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            Self::Bundle { id: Some(id), error } => write!(f, "Bundle {} error: {}", id, error),
            ...
            Self::SyncRequestInAsyncMode { id } => write!(f, "Requested {} synchronously while in async mode", id),
        }
    }
}
